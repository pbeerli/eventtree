  ...
  NxsToken &token)	/* the token used to read from `in' */
	...
	for (;;)
		...
		if (token.Equals("DATATYPE"))
			...
			if (numNewSymbols > (unsigned)maxNewStates)
				{
	...
	for (;;)
		...
		for (i = 0; i < (int)ntax; i++)
			...
			if (labels)
				...
				if (page == 0 && newtaxa)
					...
					if (page == 0)
						...
						//
						if ((int)taxonPos[positionInTaxaBlock] != i)
							{
			...
			for (currChar = firstChar; currChar < lastChar; currChar++)
				...
				if (interleaving && !ok)
					{
					if ((unsigned)lastChar < ncharTotal && j != lastChar)
						{
...
void NxsCharactersBlock::HandleTransposedMatrix(
  NxsToken &token)	/* the token used to read from in */
	...
	for (;;)
		...
		for (currChar = 0; (unsigned)currChar < ncharTotal; currChar++)
			{
			...
			if (labels)
				...
				if (page == 0 && newchar)
					...
					//
					if (positionInCharLabelsList != (unsigned)currChar)
						{
						if (page == 0)
							{
							errormsg = "Data for this character (";
			...
			for (i = firstTaxon; i < lastTaxon; i++)
				...
				if (interleaving && !ok)
					{
					if ((unsigned)lastTaxon < ntaxTotal && i != lastTaxon)
						{
